...
pub fn show(ui: &mut Ui, state: &mut AppState) {
    ...
    settings_card(ui, |ui| {
        ui.horizontal_wrapped(|ui| {
            ...
            ui.separator();
            ui.label("当前页与仪表盘/终端调试共用同一主渲染区域");
        });
    });
...
fn calc_recent_throughput(entries: &[crate::app::LogEntry], window_secs: f32) -> f32 {
    ...
    let min_t = latest - chrono::Duration::milliseconds((window_secs * 1000.0) as i64);
...
fn parse_ts_ms(ts: &str) -> Option<u64> {
    ...
    let secs = t.num_seconds_from_midnight() as u64;
...
const FIELD_COLORS: [Color32; 8] = [
    Color32::from_rgb(100, 200, 255), // 蓝
    Color32::from_rgb(120, 230, 150), // 绿
    Color32::from_rgb(255, 190, 100), // 橙
    Color32::from_rgb(200, 150, 255), // 紫
    Color32::from_rgb(255, 150, 150), // 红
    Color32::from_rgb(150, 220, 220), // 青
    Color32::from_rgb(250, 220, 130), // 黄
    Color32::from_rgb(220, 180, 200), // 粉
];
...
fn draw_frame_dissector(ui: &mut Ui, protocol: AnalyzerProtocol, bytes: &[u8]) {
    ...
    ui.label(
        RichText::new("帧结构深度解剖 / Frame Dissector")
            .strong()
            .size(13.0)
            .color(Color32::from_rgb(0, 122, 204)),
    );
...
fn dissect_serial(bytes: &[u8]) -> Vec<FrameField> {
    let mut fields = Vec::new();
    // 尝试检测常见的帧结构（如 header + payload + checksum）
    if bytes.len() >= 2 {
        // 检测是否有常见头部标识
...
fn dissect_usb(bytes: &[u8]) -> Vec<FrameField> {
    ...
    // 1) 检测 MSC BOT CBW/CSW (不以 setup packet 开头)
    if let Some(bot_type) = detect_usb_bot_frame(bytes) {
        match bot_type {
    ...
    // 2) 检测 USB 描述符响应
    if let Some(desc_type) = detect_usb_descriptor_type(bytes) {
        match desc_type {
