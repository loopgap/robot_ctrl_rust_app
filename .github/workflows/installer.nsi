...
Section "Install"
    SetOutPath "$INSTDIR"
    File "target\release\robot_control.exe"
    File "target\release\tools_suite.exe"
    File "target\release\devtools.exe"
    CreateDirectory "$SMPROGRAMS\Robot Control Suite"
    CreateShortcut "$SMPROGRAMS\Robot Control Suite\Robot Control.lnk" "$INSTDIR\robot_control.exe"
    CreateShortcut "$SMPROGRAMS\Robot Control Suite\Tools Suite.lnk" "$INSTDIR\tools_suite.exe"
    CreateShortcut "$SMPROGRAMS\Robot Control Suite\DevTools.lnk" "$INSTDIR\devtools.exe"
...
SectionEnd
Section "Uninstall"
    Delete "$INSTDIR\robot_control.exe"
    Delete "$INSTDIR\tools_suite.exe"
    Delete "$INSTDIR\devtools.exe"
    Delete "$INSTDIR\uninstall.exe"
    RMDir "$INSTDIR"
    Delete "$SMPROGRAMS\Robot Control Suite\Robot Control.lnk"
    Delete "$SMPROGRAMS\Robot Control Suite\Tools Suite.lnk"
    Delete "$SMPROGRAMS\Robot Control Suite\DevTools.lnk"
