...
impl RegexWorkbenchTool {
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui, ctx: &egui::Context, lang: Language) {
        ...
        let input_height = if wide { 210.0 } else { 180.0 };
        let output_height = if wide { 210.0 } else { 180.0 };
...
mod tests {
    ...
    #[test]
    fn test_regex_workbench_matches_expected_lines() {
        let mut tool = RegexWorkbenchTool {
            pattern: "ERROR".to_string(),
            input: "INFO ready\nERROR timeout\nERROR crc\n".to_string(),
            ..Default::default()
        };

        tool.run(Language::En);
        assert!(tool.verified);
        assert_eq!(tool.matched, 2);
        assert!(tool.output.contains("ERROR timeout"));
    }

    #[test]
    fn test_regex_workbench_reports_invalid_regex() {
        let mut tool = RegexWorkbenchTool {
            pattern: "(".to_string(),
        ...
        tool.run(Language::Zh);
        assert!(!tool.verified);
        assert!(tool.detail.contains("正则错误"));
    }
}
